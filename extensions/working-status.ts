 ...
 *   rendered as plain theme-default text (no custom coloring)
 * - Streaming working indicator: left as pi's built-in default spinner
...
export default function (pi: ExtensionAPI) {
	...
	pi.on("session_start", async (_event, c) => {
		ctx = c;
		// Note: setWorkingIndicator is intentionally not called here, so pi's
		// default spinner (and its default colors) are used.
	...
	pi.on("turn_start", async (_event, c) => {
		ctx = c;
		// Plain working text - no custom coloring.
		c.ui.setWorkingMessage(pickRandom());
	});
